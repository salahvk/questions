#!/usr/bin/env python3
"""Build complete ih_wave15_facts.py."""

from __future__ import annotations

import pprint
import textwrap
from pathlib import Path

ROOT = Path(__file__).parent
HEADER = (ROOT / "ih_wave15_facts.py").read_text(encoding="utf-8").split("# 1 — Bhakti")[0]

_g: dict = {"__file__": str(ROOT / "gen_ih15.py")}
exec((ROOT / "gen_ih15.py").read_text(encoding="utf-8"), _g)
BHAKTI: list[tuple[str, str, str]] = _g["BHAKTI"]

MONUMENTS: list[tuple[str, str]] = [
    ("താജ്മഹൽ", "ഷാജഹാൻ"),
    ("ചെങ്കോട്ട", "ഷാജഹാൻ"),
    ("ജാമാ മസ്ജിദ് (ദില്ലി)", "ഷാജഹാൻ"),
    ("ദില്ലിവാരാ മസ്ജിദ്", "ഷാജഹാൻ"),
    ("ഹുമായൂന്റെ കബർ", "ഹുമായൂൺ"),
    ("പുരാനാ ഖില", "ഹുമായൂൺ"),
    ("ഫത്തേർപൂർ സിക്രി", "അക്ബർ"),
    ("ബുലന്ദ് ദർവാസ", "അക്ബർ"),
    ("ഐതുമുദ്ദുല്ലാഹ്", "അക്ബർ"),
    ("ജോധാ ബായി കുഴി", "അക്ബർ"),
    ("ആഗ്ര കോട്ട", "അക്ബർ"),
    ("ഇത്മാദ്-ഉദ്-ദൗല", "നൂർ ജഹാൻ"),
    ("ഷാലിമാർ ബാഗ് (ശ്രീനഗർ)", "ജഹാംഗീർ"),
    ("ലാൽ ഖില", "ഷാജഹാൻ"),
    ("മോതി മസ്ജിദ്", "ഔറംഗസേബ്"),
    ("ബാദ്ഷാഹി മസ്ജിദ്", "ഔറംഗസേബ്"),
    ("ബീബി ക മഖ്ബറ", "ഔറംഗസേബ്"),
    ("ചാർമിനാർ", "മുഹമ്മദ് ഖുലി ഖുതുബ് ഷാ"),
    ("ഗോൽക്കൊണ്ട കോട്ട", "കാകത്തിയ"),
    ("ക്വുതുബ് മിനാർ", "ഖുതുബ്-ഉദ്-ദിൻ ഐബക്"),
    ("അലൈ ദർവാസ", "അലാവുദ്ദീൻ ഖൽജി"),
    ("ഹൗസ് ഖാസ്", "അലാവുദ്ദീൻ ഖൽജി"),
    ("തുഗ്ലഖാബാദ് കോട്ട", "ഗിയാസുദ്ദിൻ തുഗ്ലəq"),
    ("ഫിറോസ് ഷാ കോട്ട", "ഫിറോസ് ഷാ തുഗ്ലəq"),
    ("പുരാനാ ഖില (ദില്ലി)", "ഷെർഷാ സൂരി"),
    ("ഷെർഷാ സമാധി", "ഷെർഷാ സൂരി"),
    ("വിരുപാക്ഷ ക്ഷേത്രം", "വിജയനഗര"),
    ("വിട്ടാല ക്ഷേത്രം", "വിജയനഗര"),
    ("കോണാർക്ക് സൂര്യക്ഷേത്രം", "നarasimhadeva"),
    ("ഖജുരാഹോ", "ചന്ദെല"),
    ("മഹാബോധി ക്ഷേത്രം", "ഗുപ്തർ"),
    ("സാഞ്ചി സ്തൂപം", "അശോകൻ"),
    ("എലിഫണ്ടാ ഗുഹകൾ", "രാഷ്ട്രകൂടർ"),
    ("അജanta ഗുഹകൾ", "വാകാടകർ"),
    ("എല്ലോറ കൈലാസം", "രാഷ്ട്രകൂടർ"),
    ("മീനാക്ഷി ക്ക്ഷേത്രം", "പാണ്ഡ്യർ"),
    ("ബൃഹദീശ്വര ക്ക്ഷേത്രം", "രാജരാജ ചോളൻ"),
    ("ഗംഗൈകൊണ്ട ചോളപുരം", "രാജേന്ദ്ര ചോളൻ"),
    ("ഖുതുബ് ഷാഹി സമാധികൾ", "ഖുതുബ് ഷാഹി"),
    ("ജന്തർ മന്തർ (ജയ്പൂർ)", "സവായി ജയ്സിംഗ്"),
    ("ആംബർ കോട്ട", "കച്ചവാഹ"),
    ("ചിത്തോർഗഢ് കോട്ട", "സിസോദിയ"),
    ("ഗ്വാലിയർ കോട്ട", "തോമർ"),
    ("നിഷat ബാഗ്", "ആസഫ് ഖാൻ"),
    ("മയൂർ സിംഹാസനം", "ഷാജഹാൻ"),
    ("സഫ്ദർജംഗ് സമാധി", "സഫ്ദർജംഗ്"),
    ("ജന്തർ മന്തർ (ദില്ലി)", "സവായി ജയ്സിംഗ്"),
    ("സിറ്റി പാലace (ജയ്പൂർ)", "സവായി ജയ്സിംഗ്"),
    ("ലിംഗaraj ക്ക്ഷേത്രം", "സോംവംശി"),
    ("ഹംപി വിശ്വനാഥ ക്ക്ഷേത്രം", "വിജയനഗര"),
]

# Fix corrupted entries
FIXES = {
    "ഗിയാസുദ്ദിൻ തുഗ്ലəq": "ഗിയാസുദ്ദിൻ തുഗ്ലəq".replace("əq", "ല"),
    "ഫിറോസ് ഷാ തുഗ്ലəq": "ഫിറോസ് ഷാ തുഗ്ലəq".replace("əq", "ല"),
    "നarasimhadeva": "നarasimhadeva".replace("narasimhadeva", "നarasimhadeva"),
}
MONUMENTS = [
    (a, FIXES.get(b, b.replace("narasimhadeva", "നarasimhadeva").replace("əq", "ല")))
    for a, b in MONUMENTS
]
MONUMENTS = [
    (a.replace("narasimhadeva", "നarasimhadeva").replace("Ajanta", "അജanta").replace("Ajanta", "അജanta"), b)
    for a, b in [
        ("താജ്മഹൽ", "ഷാജഹാൻ"),
        ("ചെങ്കോട്ട", "ഷാജഹാൻ"),
        ("ജാമാ മസ്ജിദ് (ദില്ലി)", "ഷാജഹാൻ"),
        ("ദില്ലിവാരാ മസ്ജിദ്", "ഷാജഹാൻ"),
        ("ഹുമായൂന്റെ കബർ", "ഹുമായൂൺ"),
        ("പുരാനാ ഖില", "ഹുമായൂൺ"),
        ("ഫത്തേർപൂർ സിക്രി", "അക്ബർ"),
        ("ബുലന്ദ് ദർവാസ", "അക്ബർ"),
        ("ഐതുമുദ്ദുല്ലാഹ്", "അക്ബർ"),
        ("ജോധാ ബായി കുഴി", "അക്ബർ"),
        ("ആഗ്ര കോട്ട", "അക്ബർ"),
        ("ഇത്മാദ്-ഉദ്-ദൗല", "നൂർ ജഹാൻ"),
        ("ഷാലിമാർ ബാഗ് (ശ്രീനഗർ)", "ജഹാംഗീർ"),
        ("ലാൽ ഖില", "ഷാജഹാൻ"),
        ("മോതി മസ്ജിദ്", "ഔറംഗസേബ്"),
        ("ബാദ്ഷാഹി മസ്ജിദ്", "ഔറംഗസേബ്"),
        ("ബീബി ക മഖ്ബറ", "ഔറംഗസേബ്"),
        ("ചാർമിനാർ", "മുഹമ്മദ് ഖുലി ഖുതുബ് ഷാ"),
        ("ഗോൽക്കൊണ്ട കോട്ട", "കാകത്തിയ"),
        ("ക്വുതുബ് മിനാർ", "ഖുതുബ്-ഉദ്-ദിൻ ഐബക്"),
        ("അലൈ ദർവാസ", "അലാവുദ്ദീൻ ഖൽജി"),
        ("ഹൗസ് ഖാസ്", "അലാവുദ്ദീൻ ഖൽജി"),
        ("തുഗ്ലഖാബാദ് കോട്ട", "ഗിയാസുദ്ദിൻ തുഗ്ലəq"),
        ("ഫിറോസ് ഷാ കോട്ട", "ഫിറോസ് ഷാ തുഗ്ലəq"),
        ("പുരാനാ ഖില (ദില്ലി)", "ഷെർഷാ സൂരി"),
        ("ഷെർഷാ സമാധി", "ഷെർഷാ സൂരി"),
        ("വിരുപാക്ഷ ക്ഷേത്രം", "വിജയനഗര"),
        ("വിട്ടാല ക്ഷേത്രം", "വിജയനഗര"),
        ("കോണാർക്ക് സൂര്യക്ഷേത്രം", "നarasimhadeva"),
        ("ഖജുരാഹോ", "ചന്ദെല"),
        ("മഹാബോധി ക്ഷേത്രം", "ഗുപ്തർ"),
        ("സാഞ്ചി സ്തൂപം", "അശോകൻ"),
        ("എലിഫണ്ടാ ഗുഹകൾ", "രാഷ്ട്രകൂടർ"),
        ("അജanta ഗുഹകൾ", "വാകാടകർ"),
        ("എല്ലോറ കൈലാസം", "രാഷ്ട്രകൂടർ"),
        ("മീനാക്ഷി ക്ക്ഷേത്രം", "പാണ്ഡ്യർ"),
        ("ബൃഹദീശ്വര ക്ക്ഷേത്രം", "രാജരാജ ചോളൻ"),
        ("ഗംഗൈകൊണ്ട ചോളപുരം", "രാജേന്ദ്ര ചോളൻ"),
        ("ഖുതുബ് ഷാഹി സമാധികൾ", "ഖുതുബ് ഷാഹി"),
        ("ജന്തർ മന്തർ (ജയ്പൂർ)", "സവായി ജയ്സിംഗ്"),
        ("ആംബർ കോട്ട", "കച്ചവാഹ"),
        ("ചിത്തോർഗഢ് കോട്ട", "സിസോദിയ"),
        ("ഗ്വാലിയർ കോട്ട", "തോമർ"),
        ("നിഷat ബാഗ്", "ആസഫ് ഖാൻ"),
        ("മയൂർ സിംഹാസനം", "ഷാജഹാൻ"),
        ("സഫ്ദർജംഗ് സമാധി", "സഫ്ദർജംഗ്"),
        ("ജന്തർ മന്തർ (ദില്ലി)", "സവായി ജയ്സിംഗ്"),
        ("സിറ്റി പാലace (ജയ്പൂർ)", "സവായി ജയ്സിംഗ്"),
        ("ലിംഗaraj ക്ക്ഷേത്രം", "സോംവംശി"),
        ("ഹംപി വിശ്വനാഥ ക്ക്ഷേത്രം", "വിജയനഗര"),
    ]
]

print("broken - rewrite")
