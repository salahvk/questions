#!/usr/bin/env python3
"""Keep valid sports.json questions and refill with wave20 unique facts."""
...
from sports_wave20_facts import generate_wave20_candidates

try:
    from sports_gov_utils import sports_gov_body_issue
except ImportError:
    sports_gov_body_issue = None  # type: ignore[misc, assignment]

try:
    from sports_facts import FACTS as SPORTS_FACTS, OLYMPICS_HOSTS
except ImportError:
    SPORTS_FACTS = []
    OLYMPICS_HOSTS = []

FACT_BY_STEM = {q: (ans, wrong, diff) for q, ans, wrong, diff in SPORTS_FACTS}
_cities_o = [c for _, c in OLYMPICS_HOSTS if re.search(r"^[\u0D00-\u0D7F]", c)]
for _year, _city in OLYMPICS_HOSTS:
    if not re.search(r"^[\u0D00-\u0D7F]", _city):
        continue
    _stem = f"{_year} ഒളിമ്പിക്സ് ആതിഥ്യ നഗരം?"
    _wrong = [c for c in _cities_o if c != _city][:3]
    FACT_BY_STEM[_stem] = (_city, _wrong, "medium")
    ...
    if BANNED_OPTION_ENGLISH:
        for opt in options:
            if BANNED_OPTION_ENGLISH.search(opt):
                return True
    if sports_gov_body_issue is not None:
        if sports_gov_body_issue(question, answer):
            return True
    return False
...
def refresh_kept_from_facts(q: dict, rng: random.Random) -> dict:
    """Sync legacy kept rows with corrected sports_facts options."""
    stem = q.get("question", "").strip()
    fact = FACT_BY_STEM.get(stem)
    if not fact:
        return q
    ans, wrong, diff = fact
    ...
    rng.shuffle(opts)
    return {**q, "options": opts, "answer": ans, "difficulty": diff}
...
def main() -> int:
    ...
    for q in data.get("questions", []):
        ...
        if stem and stem not in kept_stems and not is_bad_kept(stem, opts, ans):
            kept.append(refresh_kept_from_facts(q, rng))
            kept_stems.add(stem)
