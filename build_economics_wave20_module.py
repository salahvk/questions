#!/usr/bin/env python3
"""Build economics_wave20_facts.py — 20 PSC categories, 2200+ unique stems."""
...
WORK_T = [
    "സാമ്പത്തിക ശാസ്ത്രജ്ഞൻ {person} ഏത് പ്രധാന രചനയുമായി ബന്ധപ്പെട്ടിരിക്കുന്നു?",
...
ALT_PFX = [
    "കേരള PSC സാമ്പത്തിക വിജ്ഞാനത്തിൽ — ",
...
def main() -> None:
    ...
    parts = [HEADER]
    for name, val in data_vars.items():
        parts.append(f"{name} = ")
    ...
    spec = importlib.util.spec_from_file_location("eco_wave20", OUT)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    n = len(mod.generate_wave20_candidates(set(), random.Random(42)))
    print(f"Candidate count (no existing): {n}")
    if n < MIN_COUNT:
        raise SystemExit(f"SHORTFALL: need {MIN_COUNT - n} more verified facts/templates")
