#!/usr/bin/env python3
"""Refill biology.json with wave-20 categories to target count (5627+)."""
...
from biology_ml_check import passes_malayalam_qc
...
PREFIX = "bio_"
TARGET = max(CATEGORY_REGISTRY["biology.json"][1], 5792)  # 1092 existing + 4700 new
...
def has_english_leak_options(options: list[str], answer: str) -> bool:
    ...
    if re.search(
        r"\b(skin|liver|lung|heart|femur|stapes|aorta|capillary|circulatory|digestive|"
        ...
        r"scurvy|rickets|beriberi|anemia|osteoporosis|goiter)\b",
        text,
        re.I,
    ):
        return True
...
def main() -> int:
    ...
    for q, opts, ans, diff in candidates:
        ...
        if q in kept_stems:
            continue
        if is_bad_kept(q, opts, ans):
            continue
        if not passes_malayalam_qc(q, opts, ans):
            continue
    ...
    for i, q in enumerate(combined[:TARGET], start=1):
        ...
        if stem in seen or is_bad_kept(stem, q.get("options", []), q.get("answer", "")):
            continue
        if not passes_malayalam_qc(stem, q.get("options", []), q.get("answer", "")):
            continue
        seen.add(stem)
        entry = dict(q)
    ...
    print(f"biology.json: {len(final)} questions (kept {len(kept)}, added {added}, target {TARGET})")
    print(f"shortfall: {max(0, TARGET - len(final))}")
